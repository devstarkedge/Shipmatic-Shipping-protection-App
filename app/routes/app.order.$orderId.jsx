...
export async function action({ request, params }) {
  ...
  try {
    ...
    return json(
      { error: `Failed to submit claim: ${error.message || "Unknown error"}` },
      { status: 500 },
    );
  }
}
...
export default function ClaimPage() {
  ...
  useEffect(() => {
    if (success) {
      ...
      setToastContent("A claim for this order already exists ");
      setToastError(true);
      setToastActive(true);
  ...
  const handleProofFileChange = (itemId, files) => {
    ...
    Promise.all(base64Promises).then((base64s) => {
      setProofFilesBase64((prev) => ({
        ...prev,
        [itemId]: [...(prev[itemId] || []), ...base64s],
      }));
    });
  };

  const hasSelectedItems = Object.values(selectedItems).some((qty) => qty > 0);
  const allSelectedHaveReasons = Object.entries(selectedItems).every(
    ([itemId, qty]) =>
      qty === 0 ||
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          <Button
                            disabled={
                              selectedItems[item.id] >= item.maxQuantity
                            }
                            onClick={() => handleQuantityChange(item.id, 1)}
                            plain
                            size="micro"
                ...
                >
                  <div
                    style={{
                      ...
                      backgroundColor:
                        selectedMethod === "reorder" ? "#F3ECF7" : undefined,
                    }}
                  ...
                  <div
                    style={{
                      ...
                      flex: "1",
                      backgroundColor:
                        selectedMethod === "refund" ? "#F3ECF7" : undefined,
                    }}
                ...
                >
                  ...
                  <Form method="post">
                    <input
                      type="hidden"
                      name="claimData"
                      value={JSON.stringify(claimData)}
                    />
                    <input type="hidden" name="method" value={selectedMethod} />
                    <Button primary submit disabled={!selectedMethod}>
                      Submit
                    </Button>
                  </Form>
                </div>
